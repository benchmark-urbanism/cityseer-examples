...
Use of the dual representation is optional, but recommended. Keep in mind that if you've converted your `networkx` network into a dual representation, then the nodes `GeoDataFrame` will contain both the dual nodes (midpoints of streets) as well as the primal edges (street geometries). The primal edges will be set to the default geometry so that outputs can be visualised using the primal street geometries.
