...
The best way to learn Python is by trying things out. Change the examples, break them, and see what happens. If you don't understand something, add a `print()` statement or try a small experiment in a new cell. The fastest way to progress is to roll up your proverbial sleeves and use the tools to solve problems and learn as you go. What follows below is a very brief and very basic introduction to Python, and if you realistically want to use it from day-to-day, you have to get hands-on and look for further resources, of which there is an abundance online. It has never been a better time to learn because large language models can speed up the process and answer most of your questions. However, be aware that they can send you down rabbit holes. Don't use them or the code they generate if you don't understand what is going on; use them instead as a way to amplify or better understand what you do, rather than to do the work completely for you. Otherwise, you will learn nothing in the process and miss the opportunities to use these tools to their greatest potential.
...
Variables can be one of several types, such as numbers (integers or floats) and text (strings). Functionality is specific to types. While some functionality is shared across types, other behaviours are type-specific. When working with variables, especially in Python, be cognisant of the type assigned to the variable. Misusing types can lead to issues and errors. For example, trying to add a string to an integer will result in an error.
...
A method is a function (which we'll cover in detail later) that belongs to an object, like a string. For example, `str.upper()` is a method of the string class that converts a string to uppercase. You access methods using dot notation (`object.method()`) and execute them by adding parentheses. If a method requires options or parameters, they go inside the parentheses.
...
print(full_name.strip())  # Remove leading/trailing spaces
...
1. Create a variable for a number. Print its type using `type()`.
2. Create a variable for your name, then print a statement using an F-string to say "Hello, [your name]!".

#### Exercise: Arithmetic and Strings
...
| Type       | Ordered (Insertion) | Unique Keys  | Mutable | Example Syntax     | Use Case Example              |
| ---------- | ------------------- | ------------ | ------- | ------------------ | ----------------------------- |
...
| Dictionary | Yes (Since Py 3.7+) | Yes (Keys)   | Yes     | `{"a": 1, "b": 2}` | Lookup tables, data records   |

### Lists: Ordered Sequences

Use lists when the order of items matters and you might need to change the items later. Lists are mutable, which means they behave like a row of mailboxes. You can put content into the list and take content out, but the mailbox stays where it is and can be updated in place. To access the contents of a list, use square brackets with the index of the content, where the index is based on zero for the first item. You can also reverse index by using a negative symbol, in which case `-1` represents the last item in the list, and so forth.
...
Use dictionaries when you need to look up values based on a unique key (like looking up a word's definition in a dictionary). Dictionaries are powerful data structures because you can use the key to reference a value. Once you have structured data in a dictionary, you can later retrieve a particular value based on an associated key. You can also update the value associated with a key. Like lists, dictionaries use square bracket notation for indexing, but in this case, instead of using an integer for the position of an item, you use the key.
