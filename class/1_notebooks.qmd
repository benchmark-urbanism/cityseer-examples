...
1. **Access Colab:** Visit [Google Colab](https://colab.research.google.com/) and sign in with a Google account.
2. **Create a New Notebook:** Click on "File > New Notebook" to start a new notebook.
3. **Upload an Existing Notebook:** If you already have a `.ipynb` file, upload it via "File > Upload Notebook."

:::

#### Resetting the Runtime

If the notebook becomes unresponsive or you need to clear all variables, use `Runtime > Restart runtime...`. After restarting, rerun all cells to restore the notebook's state.
...
Text cells use Markdown, a lightweight markup language. For example:

```markdown
### Observations

This notebook demonstrates the basics of using computational notebooks for interactive coding and documentation.
```

::: {.callout-note}

1. **Troubleshooting Errors:** Ensure all prerequisite cells have been run.
2. **Collaborating:** Share notebooks via "Share" in the top-right corner. Adjust permissions for viewing or editing.
3. **Saving Progress:** Regularly save work to Google Drive via "File > Save a Copy in Drive."
4. **Restarting the Environment:** Use "Runtime > Restart Runtime" to reset and rerun all cells.

:::

### Exercise

1. Create a new code cell and write a Python command to print the name of your favorite city. Run the cell.
2. Add a new text cell with a heading (e.g., "My Favorite City") and a brief description of why you chose it. Render the cell to format the text.
