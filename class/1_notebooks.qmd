...
A computational notebook is an interactive environment where code, results, and explanations coexist. It is ideal for iterative workflows like data analysis, allowing step-by-step testing and documentation alongside code. Setup is automatic, feedback is direct, and experimentation is intuitive, making it an approachable way to learn Python compared to older methods.

## Google Colab

Google Colab is a cloud-based platform for running notebooks without local setup. It is great for exploring datasets or prototyping workflows.
...
A notebook consists of cells, which can be code or text.
...
Run a code cell by selecting it and pressing `Shift+Enter` or clicking the play button. For example:
...
Variables persist across cells within an active session. This enables flexibility because later cells can use information from earlier cells. However, this can also lead to confusion when cells are executed out of order or when changes are made to earlier cells without re-running the later cells that depend on them.
...
Changing a variable will not update its value in memory unless the cell is re-run.
...
Text cells use [Markdown](https://www.markdownguide.org/basic-syntax/), a lightweight markup language. Markup has built-in support for formatting text through the use of characters: you can create headings, bolded text, italicised text, numbered or bulleted lists, even tables. See the linked formatting guide for more information:
